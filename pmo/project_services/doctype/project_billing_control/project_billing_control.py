...
class ProjectBillingControl(Document):
    ...
    def make_delivery_note(self,project_name,scope_item,items_value,billing_percentage,due_date,description_when,vat_value,billing_state,delivery_note,schedule_bundle_qty_name):
        arr=[]
        for row in self.project_payment_schedule_control:
            if row.invoice==1:
                arr.append(row.name)
            ...
            if arr and len(arr)==1:
                ...
                if customer:
                    ...
                    for resource in resources_details_name:
                        ...
                        print required_qty
                        print '*************'
                        rate = doc.final_selling_price/flt(required_qty)
 ...
 #      return init_payment_name
    ...
    def make_delivery_notes(self,project_name,scope_item,items_value,billing_percentage,due_date,description_when,vat_value,billing_state,delivery_note,created_done):
        arr=[]
        for row in self.project_payment_schedule_control:
            if row.invoice==1:
                arr.append(row.name)
        ...
        if delivery_note and billing_state==1:
            frappe.throw("You made Delivery Note for one of this items before")
        elif created_done!=1:
            if not delivery_note and arr and len(arr)==1:
