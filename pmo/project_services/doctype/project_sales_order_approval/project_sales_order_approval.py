...
class SalesOrder(SellingController):
	...
	def check_modified_date(self):
		mod_db = frappe.db.get_value("Project Sales Order Approval", self.name, "modified")
...
def close_or_unclose_sales_orders(names, status):
	if not frappe.has_permission("Project Sales Order Approval", "write"):
		frappe.throw(_("Not permitted"), frappe.PermissionError)

	names = json.loads(names)
	for name in names:
		so = frappe.get_doc("Project Sales Order Approval", name)
		if so.docstatus == 1:
			if status == "Closed":
...
def make_sales_invoice(source_name, target_doc=None, ignore_permissions=False):
	...
	doclist = get_mapped_doc("Project Sales Order Approval", source_name, {
		"Project Sales Order Approval": {
...
def get_events(start, end, filters=None):
	...
	conditions = get_event_conditions("Project Sales Order Approval", filters)
